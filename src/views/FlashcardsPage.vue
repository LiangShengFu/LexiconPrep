...
const hasCards = computed(() => filteredCards.value.length > 0)

const flip = () => { flipped.value = !flipped.value }
const nextCard = () => {
  flipped.value = false
  currentIndex.value = currentIndex.value < filteredCards.value.length - 1 ? currentIndex.value + 1 : 0
}
const prevCard = () => {
  flipped.value = false
  currentIndex.value = currentIndex.value > 0 ? currentIndex.value - 1 : filteredCards.value.length - 1
}
...
const createCard = async () => {
  ...
  try {
    ...
    newFront.value = ''; newBack.value = ''; newSubject.value = ''
...
<template>
  <div class="p-8 space-y-6">
    ...
    <div v-if="hasCards" class="flex flex-col items-center">
      <div class="w-full max-w-[600px] aspect-[3/2] cursor-pointer perspective-[1000px]" @click="flip">
    ...
    <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showCreate = false">
      <div class="card-xai w-full max-w-[440px] mx-4 space-y-4">
        ...
        <div>
          <label class="block text-body text-sm mb-2">正面（问题）</label>
          <input v-model="newFront" type="text" placeholder="输入问题..." class="w-full bg-canvas-soft border border-hairline rounded-card px-4 py-2.5 text-ink text-sm placeholder:text-mute focus:outline-none focus:border-ink" />
        </div>
        <div>
          <label class="block text-body text-sm mb-2">背面（答案）</label>
          <input v-model="newBack" type="text" placeholder="输入答案..." class="w-full bg-canvas-soft border border-hairline rounded-card px-4 py-2.5 text-ink text-sm placeholder:text-mute focus:outline-none focus:border-ink" />
        </div>
        <div>
          <label class="block text-body text-sm mb-2">学科（可选）</label>
