      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="card-xai space-y-3">
            <textarea v-model="newContent" placeholder="分享你的学习心得..." rows="2"
              class="w-full bg-canvas-soft border-4 border-dashed border-hairline rounded-card px-4 py-2.5 text-ink text-sm placeholder:text-mute focus:outline-none focus:border-ink resize-none max-h-[120px]" />
            <div class="flex items-center justify-between">
              <input v-model="newSubject" placeholder="标签（如：考研经验）" class="bg-canvas-soft border-4 border-dashed border-hairline rounded-card px-3 py-1.5 text-ink text-xs placeholder:text-mute focus:outline-none focus:border-ink w-40" />
          ...
          <div v-for="post in posts" :key="post.id" class="card-xai">
            <div class="flex items-center gap-3 mb-3">
              <div class="w-8 h-8 rounded-full bg-canvas-soft border-4 border-dashed border-hairline flex items-center justify-center text-xs text-ink font-mono">
              ...
              <span v-if="post.subject" class="ml-auto text-xs px-2 py-0.5 rounded-full border-4 border-dashed border-hairline text-mute font-mono uppercase">{{ post.subject }}</span>
            </div>
            <p class="text-body text-sm leading-relaxed">{{ post.content }}</p>
            <div class="flex items-center gap-4 mt-4 pt-3 border-t-4 border-dashed border-hairline">
        ...
        <div class="space-y-4">
          <div class="card-xai">
            ...
            <div v-for="(l, i) in leaders" :key="l.name" class="flex items-center gap-3 py-2 border-b-4 border-dashed border-hairline last:border-b-0">
              <span class="text-xs text-mute font-mono w-4">{{ i + 1 }}</span>
              <div class="w-6 h-6 rounded-full bg-canvas-soft border-4 border-dashed border-hairline" />
