...
const shuffleText = (originalText: string, currentRevealed: Set<number>): string => {
  if (props.useOriginalCharsOnly) {
    ...
    const availableChars = props.characters.split('')
      ...
      interval = setInterval(() => {
        if (props.sequential) {
          if (revealedIndices.value.size < props.text.length) {
            ...
            clearInterval(interval!); interval = null; isScrambling.value = false; emit('animationComplete')
          }
        } else {
          ...
          if (currentIteration >= props.maxIterations) {
            ...
            displayText.value = props.text; emit('animationComplete')
          }
        }
      }, props.speed)
    } else {
      displayText.value = props.text; revealedIndices.value = new Set(); isScrambling.value = false
    }
  }
)
...
onMounted(async () => {
  if (props.animateOn === 'view') {
    ...
    intersectionObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => { if (entry.isIntersecting && !hasAnimated.value) { isHovering.value = true; hasAnimated.value = true } })
